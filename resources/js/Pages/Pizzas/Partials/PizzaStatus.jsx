export default function PizzaStatus({pizza}) {
    const statuses = [
        'Ordered',
        'Prepping',
        'Baking',
    ...
    const currentStatus = pizza.status
    ...
    return (
        <div className={'flex border-4 border-blue-200 rounded-full'}>
            {statuses.map((status, i) => (
                <div key={i} className={getClass(status, i)}>
                    <p className={'uppercase font-medium italic text-white drop-show text-center'}>
                        <span className={'block text-3xl font-bold not-italic leading-none'}>{i + 1}</span>
                        {status}
                    </p>
                </div>
            ))}
